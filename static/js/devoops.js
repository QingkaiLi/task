...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('static/plugins/fineuploader/jquery.fineuploader-5.0.1.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	...
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('static/plugins/d3/d3.v3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadAjaxContent(url){
	...
	var curSideMenu = function(){
		var menuUl = $('#sidebar-left .main-menu'),
			hash   = location.hash;

		    hash   = hash.substring(1);

		menuUl.find('a').removeClass('active-parent active');
		var filter = function( Aobj ){
			var result = [],
				Ul     = Aobj;

			while( ! Ul.hasClass('main-menu') ){
				
				if ( Ul.prop('nodeName').toLowerCase() == 'ul' ){
		...
		var curA   = menuUl.find('[href="'+hash+'"]');
		
		// 是否能在左侧菜单中找到对应的url
		if ( curA.length ){

			var FUl = curA.closest('ul');

			curA.addClass('active-parent active');
			// 是否有子菜单
			if ( !FUl.hasClass('main-menu') ){
				// 有子级菜单的情况
				var ulArr = filter( curA );
				for(var i in ulArr){
					ulArr[i].show().prev('a').addClass('active-parent active');
				}
				
			}

		}else{
			// 特殊页面，没有对应的左侧栏目，手动配置参数映射
			url = url.match(/.*\.htm/ig);// 过滤参数部分
			for(var i in asideMenu){
				if ( asideMenu[i].hash == url ){
					var Aele  = $(asideMenu[i].current);
					var ulArr = filter( Aele );
					for(var i in ulArr){
						ulArr[i].show().prev('a').addClass('active-parent active');
					}
					Aele.addClass('active-parent active');
	...
	$.ajax({
		...
		success: function(data) {
			...
			// 查找页面中是否已经存在浮层，避免一个事件触发多个浮层展示于页面.
			var isPop = $('body > .popBox');
			if( isPop.length ){
				isPop.remove();
			};
...
function DrawKnobDashboard(){
	...
	console.log(srv_monitoring_selectors);
...
$(document).ready(function () {
	...
	/* 
		...
		Modify:tanshenghu
	*/
	GoToPageAlink = $('<a id="GoToPageAlink" href="#">&nbsp;</a>');
	GoToPageAlink.on('click', function(ev){
		ev.preventDefault();
...
/* 
	↓ Modify:tanshenghu
*/
var ajaxGoToPage = function( url ){
	var _blank = arguments[1];
	if ( GoToPageAlink.prop('nodeName').toLowerCase()!=='a' ) throw '不能调用该方法，变量已经被篡改了。';
	if ( _blank ){
		GoToPageAlink.attr('target', _blank);
		window.open( url );
		return;
	} else {
		GoToPageAlink.removeAttr('target');
	}
	if ( url ){
		GoToPageAlink.attr('href', url).trigger('click');
	}
};

var asideMenu = [
/*
	有一些页面，没有直接对应的左侧，所以需要手动配置一些参数，如下
	例子：
	{'hash':'文件url路径', 'current':'映射对应的栏目'}
*/
	{'hash':'show/showEdit.htm', 'current':'#sidebar-left .main-menu >li:eq(0) > .dropdown-menu > li:eq(5) .ajax-link'},
	{'hash':'cinema/similarCinemaList.htm', 'current':'#sidebar-left .main-menu >li:eq(0) > .dropdown-menu > li:eq(1) .ajax-link'}
];
